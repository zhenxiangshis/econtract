...
namespace SQLServerDAL
{
	...
	public partial class UserType:IUserType
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("UserTypeID", "UserType"); 
		}
		...
		public Model.UserType GetModel(int UserTypeID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 UserTypeID,Type from UserType ");
			strSql.Append(" where UserTypeID=@UserTypeID");
			SqlParameter[] parameters = {
					new SqlParameter("@UserTypeID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Model.UserType DataRowToModel(DataRow row)
		{
			Model.UserType model=new Model.UserType();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select UserTypeID,Type ");
