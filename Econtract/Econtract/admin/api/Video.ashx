...
public class Video : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        int iTotalRecords = System.Convert.ToInt32(DbHelperSQL.ExecuteSqlGet("SELECT count(*) FROM [Video_Info] where  AddTime > '2010' ", ""));
        int iDisplayStart = 1;
        int iDisplayLength = int.Parse(context.Request.QueryString["iDisplayLength"].ToString());
        string[] aColumns = new string[] { "", "VideoID", "Title", "AddTime", "", "", "" };
        int iSortingCols = 1;
        int sSortDir = 1;
        ...
        //获取客户端的Cookie对象
        HttpCookie cok = context.Request.Cookies["Video_List"];
        ...
        if (!String.IsNullOrEmpty(context.Request.QueryString["sSearch"]))
        {
            ...
            strWhere += " and (Charindex('" + _s + "',VideoID) > 0 or Charindex('" + _s + "',Title) > 0 )";
        }

        DataSet set = new DataSet();
        Video_Info Videobll = new Video_Info();
        set = Videobll.GetVideoInfoList(iDisplayLength, iDisplayStart, aColumn, sSortDir, ref IsReCount, strWhere);
        ...
        {
            data.AppendFormat(",[\"{4}\",\"{0}\",\"{1}\",\"{2}\",\"{3}\",\"{5}\",\"{6}\",\"{7}\"]", row["VideoID"].ToString(), row["Title"].ToString().Replace('"', ' ').Replace(',', '，'), DateTime.Parse(row["AddTime"].ToString()).ToString("yyyy-MM-dd HH:mm:ss"), GcName(System.Convert.ToInt32(row["ClassID"])), row["GoUrl"].ToString() != "" ? row["GoUrl"].ToString() : "/NewHtml/" + Utility.StringHelper.DateToYear(row["AddTime"].ToString()) + "/" + row["VideoID"].ToString() + ".html", row["IsTop"].ToString(), row["IsVouch"].ToString(), row["ClassID"].ToString());
        }
    ...
    {
        Video_Class class2 = new Video_Class();
        return class2.GetVideoClassModel(ClassID).ClassName.ToString();
    }
