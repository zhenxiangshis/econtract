...
public class Product : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        int iTotalRecords = System.Convert.ToInt32(DbHelperSQL.ExecuteSqlGet("SELECT count(*) FROM [Article_Info] where  IsDelete=0 ", ""));
        int iDisplayStart = 1;
        ...
        if (!String.IsNullOrEmpty(context.Request.QueryString["sSearch"]))
        {
            ...
            strWhere += " and (Charindex('" + _s + "',ArticleID) > 0 or Charindex('" + _s + "',Title) > 0 )";
        }
