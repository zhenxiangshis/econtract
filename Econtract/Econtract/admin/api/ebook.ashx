...
public class Product : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        int iTotalRecords = System.Convert.ToInt32(DbHelperSQL.ExecuteSqlGet("SELECT count(*) FROM [eBookiItem] where status != -1", ""));
        int iDisplayStart = 1;
        int iDisplayLength = int.Parse(context.Request.QueryString["iDisplayLength"].ToString());
        string[] aColumns = new string[] { "itemID", "title", "hits", "createTime", "" };
        int iSortingCols = 0;
        int sSortDir = 1;
        string aColumn = "itemID";
        ...
        if (!String.IsNullOrEmpty(context.Request.QueryString["sSearch"]))
        {
            ...
            strWhere = " and (Charindex('" + _s + "',itemID) > 0 or Charindex('" + _s + "',title) > 0) ";
        }
        ...
        {
            data.AppendFormat(",[\"{0}\",\"{1}\",\"{2}\",\"{3}\",\"{4}\"]", row["itemID"].ToString(), row["title"].ToString(), row["hits"].ToString(), DateTime.Parse(row["createTime"].ToString()).ToString("yyyy-MM-dd"), GcName(System.Convert.ToInt32(row["ebookID"])));
        }
    ...
    public string GcName(int id)
    {
        ...
        return DbHelperSQL.ExecuteSqlGet("SELECT [ebookName] FROM [eBookInfo] where [ebookID] = " + id, "").ToString();
    }
    ...
    public DataSet GetList(int PageSize, int PageIndex, string OrderfldName, int OrderType, ref int IsReCount, string strWhere)
    {
        ...
        parameters[1].Value = "[itemID],[ebookID],[title],[itemContent],[hits],[status],[createTime],[tag],[imgurl]";
