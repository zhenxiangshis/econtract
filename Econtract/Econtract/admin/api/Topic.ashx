...
public class Topic : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        int iTotalRecords = System.Convert.ToInt32(DbHelperSQL.ExecuteSqlGet("SELECT count(*) FROM [Topic_Info] where  AddTime > '2010' ", ""));
        int iDisplayStart = 1;
        int iDisplayLength = int.Parse(context.Request.QueryString["iDisplayLength"].ToString());
        string[] aColumns = new string[] { "", "TopicID", "Title", "AddTime", "", "", "" };
        int iSortingCols = 1;
        int sSortDir = 1;
        ...
        //获取客户端的Cookie对象
        HttpCookie cok = context.Request.Cookies["Topic_List"];
        ...
        if (!String.IsNullOrEmpty(context.Request.QueryString["sSearch"]))
        {
            ...
            strWhere += " and (Charindex('" + _s + "',TopicID) > 0 or Charindex('" + _s + "',Title) > 0 )";
        }

        DataSet set = new DataSet();
        Topic_Info Topicbll = new Topic_Info();
        set = Topicbll.GetList(iDisplayLength, iDisplayStart, aColumn, sSortDir, ref IsReCount, strWhere);
        ...
        {
            data.AppendFormat(",[\"{0}\",\"{1}\",\"{2}\",\"{3}\",\"{4}\"]", row["TopicID"].ToString(), row["Title"].ToString().Replace('"', ' ').Replace(',', '，'), DateTime.Parse(row["AddTime"].ToString()).ToString("yyyy-MM-dd HH:mm:ss"), row["Url"].ToString(),row["BackColor"].ToString());
        }
